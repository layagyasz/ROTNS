...
{
    public class ComputerPlayer : Agent
    {
        ...
        {
            if (Administration is RegionAdministration)
            {
                Trade T = ((RegionAdministration)Administration).Region.BestTrade();
                if (T.Flow1 > 0 && T.Amount1 > 0 && T.Amount2 > 0)
                {
                    Agent A = ((Region)T.Zone2).Administration.Agent;
                    if (A.ProposeTrade(this, T))
                    {
                        T.Zone1.AddTrade(T);
                        T.Zone2.AddTrade(T);
                    }
