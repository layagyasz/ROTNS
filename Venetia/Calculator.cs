...
namespace Venetia
{
    class Calculator
    {
        public static Pair<double, double> OptimumSupply(Pair<EconomicAttributes, double>[] Input, Pair<EconomicAttributes, double>[] Output, double Population)
        {
            ...
            double Profit = 0;
            for (int i = 0; i < Output.Length; ++i) { double a = Output[i].Second * X * Output[i].First.SupplyPrice(Output[i].Second * X, Population); if (a > 0) Profit += a;}
        ...
        {
            double R = 0;
            for (int i = 0; i < Output.Length; ++i) R += DProduction(X, Output[i].First.LivingStandard, Output[i].First.Supply, Output[i].First.Demand, Output[i].Second, Output[i].First.Exponent, Output[i].First.Decay * Output[i].First.Coefficient * Population);
        ...
        {
            double R = 0;
            for (int i = 0; i < Output.Length; ++i) R += DDProduction(X, Output[i].First.LivingStandard, Output[i].First.Supply, Output[i].First.Demand, Output[i].Second, Output[i].First.Exponent, Output[i].First.Decay * Output[i].First.Coefficient * Population);
            if (Double.IsNaN(R)) return R;
        ...
        {
            return P * P * Q * X / (S - E + P * X) / Math.Log(K) + Q * P * Math.Log((S - E + P * X) / M) / Math.Log(K);
        }

        static double DDProduction(double X, double Q, double S, double E, double P, double K, double M)
        {
            return P * P * (P * X - 2 * E + 2 * S) / (Math.Log(K) * (P * X - E + S) * (P * X - E + S));
        }
        ...
        public static Triplet<double, double, double> OptimumTrade(EconomicAttributes G0L0, EconomicAttributes G0L1, EconomicAttributes G1L0, EconomicAttributes G1L1, EconomicAttributes Labor, double P0, double P1)
        {
            ...
            for (int i = 0; i < 20; ++i)
            {
                double C = X * G0L1.SupplyPrice(X, P1);
                Xi = OptimumTradeAux(C, G1L1, P1);
                X = X - (DProduction(Xi, G1L0.LivingStandard, G1L0.Supply, G1L0.Demand, 1, G1L0.Exponent, G1L0.Decay * G1L0.Coefficient * P0) + DProduction(X, G0L0.LivingStandard, G0L0.Supply, G0L0.Demand, -1, G0L0.Exponent, G0L0.Decay * G0L0.Coefficient * P1) + DProduction(Math.Max(Xi, X), Labor.LivingStandard, Labor.Supply, Labor.Demand, -.001, Labor.Exponent, Labor.Decay * Labor.Coefficient * P0))
                    / (DDProduction(Xi, G1L0.LivingStandard, G1L0.Supply, G1L0.Demand, 1, G1L0.Exponent, G1L0.Decay * G1L0.Coefficient * P0) + DDProduction(X, G0L0.LivingStandard, G0L0.Supply, G0L0.Demand, -1, G0L0.Exponent, G0L0.Decay * G0L0.Coefficient * P1) + DDProduction(Math.Max(Xi, X), Labor.LivingStandard, Labor.Supply, Labor.Demand, -.001, Labor.Exponent, Labor.Decay * Labor.Coefficient * P0));
            }
