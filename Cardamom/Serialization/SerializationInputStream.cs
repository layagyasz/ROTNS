...
namespace Cardamom.Serialization
{
    public class SerializationInputStream : SerializationStream
    {
        ...
        public void ReadCollection<T>(ICollection<T> Collection, Func<T> Deserializer)
        {
            int l = ReadInt32();
            for (int i = 0; i < l; ++i) Collection.Add(Deserializer.Invoke());
        }

        public void ReadCollection<T>(ICollection<T> Collection, Func<SerializationInputStream, T> Deserializer)
        {
            int l = ReadInt32();
            for (int i = 0; i < l; ++i) Collection.Add(Deserializer.Invoke(this));
        }
    }
}
